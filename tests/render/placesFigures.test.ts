...
// (PIOTR, 24.09; v53). From v55 the men go round their job's machines a half hour at a time, each
// one machine further on than the man hired before him, so two men want the saw in the same
// half hour only when their turns fall together: the tests here that stand several men at the
// saws put them there by hand, the way the day plan does when the turns do fall together, and
// read the places and the cells off that.
...
/** Stands these men at the saws, the way the day plan does in a half hour their turns fall on the
 *  saw together (v55): the places and the cells are read off the stations. */
function atTheSaw(state: GameState, men: string[]): void {
  for (const worker of state.workers) {
    if (!men.includes(worker.id)) continue;
    worker.working = true;
    worker.station = machineStation('tableSaw');
  }
}
...
describe('the men at their places (CLAUDE.md T25 2.6)', () => {
  it('stands two men at a two place saw at its two places', () => {
    ...
    atTheSaw(state, ['staff-1', 'staff-2']);
  ...
  it('stands three men at an industrial bench at three places of its front row', () => {
    ...
    // The hall's benches are taken away but one industrial of three places, and the saw with
    // them, so the bench is where all three go this half hour (v55).
    state.equipment = state.equipment.filter((item) => item.specId !== 'workbench' && item.specId !== 'tableSaw');
  ...
  it('spreads four men over a one place saw and the benches, and stands a man at home only when every place is taken', () => {
    ...
    // work at the benches, each at a place and a cell of his own: the saw is the second man's
    // turn this half hour, the fourth finds it taken and takes a bench (v55).
    ...
    // One bench of one place left: the bench is the first man's and the saw the second's, and
