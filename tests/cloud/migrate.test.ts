...
describe('a v29 save in this build (PIOTR, 24.09; v54)', () => {
  /** A whole hall saved on v53 with the three handling families of old: a pallet truck and a better
   *  forklift in the hall, a forklift on the lorry, and a van. */
  function lifted29(): GameState {
    const before = twoMenOnSheetWork();
    const raw = JSON.parse(JSON.stringify(before)) as Record<string, unknown> & {
      version: number;
      equipment: Array<Record<string, unknown>>;
    ...
    raw.version = 29;
    const first = raw.equipment[0];
    if (first === undefined) throw new Error('the hall has no equipment to copy');
    for (const item of raw.equipment) delete item.milesDriven;
    raw.equipment.push({ ...first, id: 'kit-pt', specId: 'palletTruck', variantId: 'standard', spriteKey: 'palletTruck' });
    raw.equipment.push({ ...first, id: 'kit-fb', specId: 'forkliftBetter', variantId: 'standard', spriteKey: 'forkliftBetter' });
    raw.equipment.push({ ...first, id: 'kit-van', specId: 'van', variantId: 'standard', spriteKey: 'van' });
    raw.onOrder.push({
      id: 'order-fl',
      specId: 'forkliftBetter',
      variantId: 'standard',
      pricePaid: 12000,
      orderedDay: 1,
      dueDay: 5,
    ...
    const after = migrateState(raw, 29);
    if (after === null) throw new Error('the lift refused a version 29 state');
    return after;
  }
  ...
  it('makes the pallet truck and the better forklift classes of the one family', () => {
    ...
    expect(byId.get('kit-pt')?.spriteKey).toBe('forklift');
    expect(after.onOrder.find((item) => item.id === 'order-fl')?.specId).toBe('forklift');
    expect(after.onOrder.find((item) => item.id === 'order-fl')?.variantId).toBe('pro');
  });

  it('puts nought miles on every van and everything else, and keeps the van the standard one', () => {
    const after = lifted29();
    for (const item of after.equipment) expect(item.milesDriven, item.id).toBe(0);
    expect(after.equipment.find((item) => item.id === 'kit-van')?.variantId).toBe('standard');
  });
});

describe('a v30 save in this build (PIOTR, 24.09; v55)', () => {
  /** A hall saved on v54 with a job half made under the old stages: a bag of Machining on it, and
   *  a bag of Finishing on a job that is not lacquered. */
  function lifted30(): { before: GameState; after: GameState } {
    const before = twoMenOnSheetWork();
    const raw = JSON.parse(JSON.stringify(before)) as Record<string, unknown> & {
      version: number;
      jobs: Array<Record<string, unknown>>;
    ...
    const value = job.labourValue as number;
    job.labourRemaining = value * 0.5;
    job.finish = 'laminate';
    job.stageLabour = { cutting: value * 0.25, machining: value * 0.15, finishing: value * 0.1 };
    const after = migrateState(raw, 30);
    if (after === null) throw new Error('the lift refused a version 30 state');
    return { before, after };
  }
  ...
  it('empties the bags of the stages that are gone and loses nothing of the job', () => {
    ...
    // The labour stays in the job: half of it done, and the bar reads the half off the new
    // quarters, the loose labour poured in order, cutting first.
    expect(job.labourRemaining).toBeCloseTo(job.labourValue * 0.5, 10);
    expect(stagePlanFor(after, job).map((stage) => stage.id)).toEqual(['cutting', 'edging', 'moulding', 'assembly']);
    expect(currentStage(after, job)?.id).toBe('moulding');
  });

  it('keeps the Finishing bag of a lacquered job, which still has the stage', () => {
    const before = twoMenOnSheetWork();
    const raw = JSON.parse(JSON.stringify(before)) as Record<string, unknown> & {
      version: number;
      jobs: Array<Record<string, unknown>>;
    ...
    if (job === undefined) throw new Error('a job is wanted here');
    job.finish = 'lacquer';
    job.stageLabour = { cutting: 10, finishing: 5 };
    const after = migrateState(raw, 30);
    expect(after?.jobs[0]?.stageLabour).toEqual({ cutting: 10, finishing: 5 });
  });
});

describe('a v55 save with its spray booths at their old size (PIOTR, 25.09; v56)', () => {
  /** The day 1 hall with a used booth in the open and a standard one against the right hand wall,
   *  where the 4 by 3 of floor the standard class held until v56 fitted and its 5 by 4 of v56 runs
   *  off the floor, saved as state version 31. */
  function hallOfV55(): { raw: Record<string, unknown>; used: { x: number; y: number } } {
    const state = buyStartingKit(newGame({ difficulty: 'veryEasy' }));
    const used = firstFreeCell(state, 'sprayBooth', 'used');
    if (used === null) throw new Error('no room for a used booth');
    placeEquipment(state, 'sprayBooth', { variantId: 'used', x: used.x, y: used.y, id: 'kit-booth-used' });
    placeEquipment(state, 'sprayBooth', {
      variantId: 'standard',
      x: state.unit.widthCells - 4,
      y: 6,
      id: 'kit-booth-standard',
    });
  ...
  it('stands a booth that no longer fits on the first free place that holds it, and leaves the one that fits', () => {
    ...
    // The used booth's floor did not move, so it stays where it stood.
    expect([booth('kit-booth-used').anchorX, booth('kit-booth-used').anchorY]).toEqual([used.x, used.y]);
    // The standard one is inside the hall on floor that holds its 5 by 4, and nothing overlaps.
    const standard = booth('kit-booth-standard');
