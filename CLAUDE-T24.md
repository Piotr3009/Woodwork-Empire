...
Precondition. main carries Turn 23 merged and the chat fixes v37 to v50: APP_VERSION 'v50',
STATE_VERSION 26 (v50: the service on the calendar, `servicedDay` on every machine,
`lastContractOfferDay` on the state). If APP_VERSION is not 'v50', stop and report.
...
- APP_VERSION = 'v51'. STATE_VERSION bumps to 27 in phase A, once, for 2.1; every v25 and v26 save loads.
...
`flexiSystem` book their hours while the extraction runs, against the endurance their class has,
and are serviced exactly as an extractor is since T23 2.8 and v50: every six months on the
calendar (`SERVICE_INTERVAL_DAYS` from `servicedDay`), the same card button, the same day out.
`isServiced` takes them in. Done: the engine test (a system bought today comes due six months on
and not before, whatever it ran; the call takes it out for the day), the Machines page test.
...
One agent, serial, in the order of section 5. Phase A first (STATE_VERSION 27, the migration,
...
STATE_VERSION 27. `dayStats.byMan` is added, `{}` in an old save; nothing else changes. Every
v25 and v26 save loads, the three fixtures in tests/fixtures (day115, day128, day149) among them.
...
T24-A1 Housekeeping and v51: docs/turn-23-brief.md byte for byte from main's CLAUDE.md, this
file as CLAUDE.md, APP_VERSION 'v51', STATE_VERSION 27 and the migration of section 4.
...
- No change to what a production minute is worth, to the stage shares, to the bag of work, to the
  contract prices, to the walk speed, to the bench rule of v47, to the by hand rule of 9.5, to the
  service rule of v50 (six months on the calendar) or to the weekly offer of v50.
...
- A dust system bought today is due a service six months on and not before, asserted.
